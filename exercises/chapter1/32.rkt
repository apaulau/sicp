...
;; preceding terms and a null-value that specifies what base value to use when the terms run out. Write accumulate and 
...
(check-equal? (factorial 5) 120) ; means that product is a special case of accumulate procedure

(define (sum term a next b)
  (accumulate + 0 term a next b))
  
(define (cube x) (* x x x))
(check-equal? (sum cube 1 inc 10) 3025) ; means that sum is a special case of accumulate procedure